  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
        "bg-[#0c1022]/90 backdrop-blur-md shadow-md" 
      }`}
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
        ...
        <nav className="hidden md:flex flex-1 justify-center">
          <ul className="flex gap-8 text-white font-semibold text-lg">
            {["Home", "Games", "Reviews", "Shop", "Contact"].map((item, i) =>
              item === "Games" ? (
      ...
      >
        ...
        <nav className="flex flex-col p-6 space-y-4">
          <Link to="/" onClick={() => setMenuOpen(false)} className="hover:text-pink-400 transition">
            Home
          </Link>
          <div>
            <Link to="/games" onClick={() => setMenuOpen(false)} className="hover:text-pink-400 transition">
              Games
            </Link>
            <Link to="/game-single" onClick={() => setMenuOpen(false)} className="block ml-4 text-sm hover:text-pink-400 transition">
              Game Single
            </Link>
          </div>
          <Link to="/reviews" onClick={() => setMenuOpen(false)} className="hover:text-pink-400 transition">
            Reviews
          </Link>
          <Link to="/shop" onClick={() => setMenuOpen(false)} className="hover:text-pink-400 transition">
            Shop
          </Link>
          <Link to="/contact" onClick={() => setMenuOpen(false)} className="hover:text-pink-400 transition">
            Contact
          </Link>
          <div className="pt-4 border-t border-gray-600">
            <Link to="/login" onClick={() => setMenuOpen(false)} className="hover:text-pink-400">
              Login
            </Link>
            <span className="mx-2">/</span>
            <Link to="/register" onClick={() => setMenuOpen(false)} className="hover:text-pink-400">
              Register
            </Link>
          </div>
        </nav>
